...
class TensorboardSummary(object):
    ...
    def visualize_image(self, opts, writer, dataset, left_image, right_image, target, seg_left, seg_right, global_step, pred_disp, gt_disp, warped_right=None):
        if opts.train_semantic and opts.train_disparity:
            ...
            grid_image = make_grid(left_image[:3].clone().cpu().data, IMAGE_COUNT, normalize=True)
            writer.add_image('Left Image', grid_image, global_step)

            grid_image = make_grid(right_image[:3].clone().cpu().data, IMAGE_COUNT, normalize=True)
            writer.add_image('Right Image', grid_image, global_step)

            grid_image = make_grid(pred_disp[:3].clone().cpu().data, IMAGE_COUNT, normalize=True)  # normalize=False?
            writer.add_image('Predicted Disparity map', grid_image, global_step)

            grid_image = make_grid(gt_disp[:3].clone().cpu().data, IMAGE_COUNT, normalize=True)  # normalize=False?
            ...
            grid_image = make_grid(decode_seg_map_sequence(torch.max(seg_left[:3], 1)[1].detach().cpu().numpy(),
                                                           dataset=dataset), IMAGE_COUNT, normalize=False, range=(0, 255))
            ...
            # grid_image = make_grid(warped_right[:3].clone().cpu().data, IMAGE_COUNT, normalize=True)  # normalize=False?
