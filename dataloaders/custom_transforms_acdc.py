...
class RandomSquareCropAndScale:
    def __init__(self, wh, mean, ignore_id, min=.5, max=2., class_incidence=None, class_instances=None,
        ...
        self.random_gens_lf = [self._rand_location_lf]
    ...
    def _rand_location_lf(self, W, H, target_w, target_h, *args, **kwargs):
        try:
            w = np.random.randint(100, 1971 - target_w + 1)
            h = np.random.randint(370, 904 - target_h + 1)
        except ValueError:
            print(f'Exception in RandomSquareCropAndScale: {target_w}')
            w = h = 0
        # left, upper, right, lower)
    ...
    def __call__(self, sample):
        ...
        if self.new_crop:
            if (sample['is_lf_data']):
                crop_fn = random.choice(self.random_gens_lf)
        ...
        ret_dict = {}
        ret_dict['left'] = self._trans(left, crop_box, target_size, pad_size, RESAMPLE, self.mean)
...
def crop_and_scale_disp_loss_weight(img: pimg, crop_box, target_size, pad_size, resample, blank_value):
    target = pimg.new(img.mode, pad_size, color=blank_value)
    target.paste(img)
    tmp = target.crop(crop_box)
    # res = tmp.resize(target_size, resample=resample)
    ...
    for d_range in depth_range:
        if d_range == 20:
            disp_mask = (disp >= 483 / d_range)  # focal_length*base_line : 2300pixel * 0.21m
        else:
...
# h_max, h_min, w_max, w_min = -1., 1024., -1, 2048.
class CropBlackArea:
    def __call__(self, sample):
        ...
        #     h_max_ = np.where(np.array(sample['label']) == 19)[0].max()  # height
        #     h_min_ = np.where(np.array(sample['label']) == 19)[0].min()
        #     w_max_ = np.where(np.array(sample['label']) == 19)[1].max()  # width
        #     w_min_ = np.where(np.array(sample['label']) == 19)[1].min()
        #     if (h_max < h_max_):
        #         h_max = h_max_
        #     if (h_min > h_min_):
        #         h_min = h_min_
...
class GammaCorrection:
    ...
    def __call__(self, sample):
        if 'weather' in sample:
            if sample['weather'] == 1 : # in night weather
                left_img = np.array(sample['left'])
                cv_img = left_img[:, :, ::-1].copy()
                res = cv2.LUT(cv_img, self.lookUpTable)
                sample['left'] = Image.fromarray(cv2.cvtColor(res, cv2.COLOR_BGR2RGB))
...
class Tensor:
    ...
    def __call__(self, example):
        ret_dict = {}
        for k in ['left', 'image_next', 'image_prev']:
            if k in example:
        ...
        if 'weather' in example:
            ret_dict['weather'] = torch.from_numpy(example['weather'])
