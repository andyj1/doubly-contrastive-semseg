...
__all__ = ["Compose", "ToTensor", "ToPILImage", "Normalize", "Resize", "Scale", "CenterCrop", "Pad",
           ...
           "RandomPerspective", "RandomErasing", "RandomCrop_PIL"]
...
class Resize(object):
    ...
    def __init__(self, size, interpolation=pimg.BILINEAR):
...
class RandomCrop2(object):
    ...
    def __call__(self, sample):
        ...
        if 'pseudo_disp' in sample.keys():
            sample['pseudo_disp'] = np.array(sample['pseudo_disp'])
        ...
        if 'disp' in sample.keys():
            sample['disp'] = Image.fromarray(sample['disp'])
        if 'pseudo_disp' in sample.keys():
            sample['pseudo_disp'] = Image.fromarray(sample['pseudo_disp'])
        if 'label' in sample.keys():
            sample['label'] = Image.fromarray(sample['label'])
...
class RandomPerspective(object):
    ...
    def __init__(self, distortion_scale=0.5, p=0.5, interpolation=pimg.BICUBIC):
        self.p = p
        self.interpolation = interpolation
...
class ReSizeImage:
    ...
    def __call__(self, sample):
        sample['left'] = sample['left'].resize(self.target_size, Image.BILINEAR)
        sample['right'] = sample['right'].resize(self.target_size, Image.BILINEAR)
        sample['disp'] = sample['disp'].resize(self.target_size, Image.BILINEAR).point(lambda i: i * (self.target_size[0]/sample['disp'].size[0]))
        if 'pseudo_disp' in sample:
            sample['pseudo_disp'] = sample['pseudo_disp'].resize(self.target_size, Image.BILINEAR).point(
                lambda i: i * (self.target_size[0] / sample['pseudo_disp'].size[0]))

        sample['label'] = sample['label'].resize(self.target_size, Image.NEAREST)
...
class PadImage:
    ...
    def __call__(self, sample):
        ...
        """
        assert sample['left'].size == sample['label'].size, 'size of img and lbl should be the same. %s, %s' % (
        sample['left'].size, sample['label'].size)
        assert sample['left'].size == sample['disp'].size, 'size of img and disp should be the same. %s, %s' % (
            sample['left'].size, sample['disp'].size)

        # pad the width if needed
        if sample['left'].size[0] < self.size[0] and sample['left'].size[1] < self.size[1]:
            sample['left'] = F.pad(sample['left'], padding=(int(self.size[0] - sample['left'].size[0]), int(self.size[1] - sample['left'].size[1]), 0, 0))
            sample['right'] = F.pad(sample['right'], padding=(int(self.size[0] - sample['right'].size[0]), int(self.size[1] - sample['right'].size[1]), 0, 0))
            sample['label'] = F.pad(sample['label'], padding=(int(self.size[0] - sample['label'].size[0]), int(self.size[1] - sample['label'].size[1]), 0, 0), fill=255)
            sample['disp'] = F.pad(sample['disp'], padding=(int(self.size[0] - sample['disp'].size[0]), int(self.size[1] - sample['disp'].size[1]), 0, 0))
            sample['pseudo_disp'] = F.pad(sample['pseudo_disp'], padding=(
            int(self.size[0] - sample['pseudo_disp'].size[0]), int(self.size[1] - sample['pseudo_disp'].size[1]), 0, 0))
