...
from dataloaders.datasets import VOCSegmentation, Cityscapes, CityLostFound, LostFound, ACDC, ACDC_City
# from dataloaders import custom_transforms as sw
from dataloaders import custom_transforms2 as sw
from dataloaders import custom_transforms_acdc as tr_acdc
...
class TwoCropTransform:
	...
	def __call__(self, sample): # lbl == label map		
...
def get_dataset(opts):
	...
	if opts.dataset == 'cityscapes':
		...
		train_tech = [tr_acdc.RandomSquareCropAndScale(random_crop_size, ignore_id=255, mean=mean_rgb),
					  tr_acdc.SetTargetSize(target_size=target_size_crops, target_size_feats=target_size_crops_feats),
					  tr_acdc.LabelBoundaryTransform(num_classes=opts.num_classes, reduce=True)]
		val_tech = [tr_acdc.FixedResize(target_size)]

		if opts.use_gamma_correction:
			train_tech += [tr_acdc.GammaCorrection()]
			val_tech += [tr_acdc.GammaCorrection()]
		train_tech += [tr_acdc.Tensor()]
		val_tech += [tr_acdc.Tensor()]

		train_transform = tr_acdc.Compose(train_tech)
		val_transform = tr_acdc.Compose(val_tech)
		
		if 'supcon' in opts.criterion: # pretraining stage: train with contrastive learning
			train_transform = TwoCropTransform(train_transform)

		train_dst = ACDC(root=opts.data_root, dataset_name=opts.dataset,
							   mode='train', transform=train_transform, opts=opts)
		val_dst = ACDC(root=opts.data_root, dataset_name=opts.dataset,
							 mode='val', transform=val_transform, opts=opts)
		if opts.use_test_data:
			val_dst = ACDC(root=opts.data_root, transform=val_transform, 
					dataset_name=opts.dataset, mode='test', opts=opts) # test case
			opts.test_dst = val_dst # append to opts to return
		else:
			opts.test_dst = val_dst
		opts.train_dst = train_dst

	elif opts.dataset == 'acdc_city':
		random_crop_size = (768, 768)
		...
		train_dst = ACDC_City(root=opts.data_root, dataset_name=opts.dataset,
							   mode='train', transform=train_transform, opts=opts)
		val_dst = ACDC_City(root=opts.data_root, dataset_name=opts.dataset,
							 mode='val', transform=val_transform, opts=opts)
