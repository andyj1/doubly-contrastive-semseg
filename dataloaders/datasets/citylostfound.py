...
class CityLostFound(data.Dataset):
    ...
    **Parameters:**
    ...
    def __init__(self, root: Path, dataset_name='cityscapes', mode='test', save_filename=False, transform=None):
        self.root = root
        self.mode = mode
        ...
        self.debug = False
        if self.debug:
            # mini dict for debugging
            city_lost_dict = {
                'train': 'filenames/debug/city_lost_train_debug.txt',
                'val': 'filenames/debug/city_lost_train_debug.txt',
                'test': 'filenames/debug/city_lost_train_debug.txt'
            }
        else:
            city_lost_dict = {
                'train': 'filenames/city_lost/city_lost_train.txt',
                'val': 'filenames/city_lost/city_lost_val.txt',
                'test': 'filenames/city_lost/city_lost_test.txt'
            }
        ...
        self.samples = []
        dataset_dict = {'city_lost': city_lost_dict, }
    ...
    def decode_target(cls, target):
        target[target == 255] = cls.num_classes
    ...
    def __getitem__(self, index):
        ...
        if sample_path['label'] is not None:
            if self.dataset_name == "city_lost":
                if self.debug:
                    target = Image.open(sample_path['label'])
                    _tmp = np.array(target, dtype=np.uint8)
                    if self.mode == "train":
                        if index < 15:  # lostandfound
                            _tmp = self.relabel_lostandfound(_tmp)
                        else:  # cityscapes
                            _tmp = self.encode_target(_tmp)
                    elif self.mode == 'val':
                        if index < 15:  # lostandfound
                            _tmp = self.relabel_lostandfound(_tmp)
                        else:  # cityscapes
                            _tmp = self.encode_target(_tmp)
                    sample['label'] = Image.fromarray(_tmp.astype('uint8'))
                else:
                    target = Image.open(sample_path['label'])
                    _tmp = np.array(target, dtype=np.uint8)
                    if self.mode == "train":
