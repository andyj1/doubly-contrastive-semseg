...
class _UpsampleBlend(nn.Module):
    ...
    def forward(self, x, skip):
        if self.detach_skip:
            ...
            skip = skip.detach()
        skip_size = skip.size()[2:4]
        # skip_size = skip.size()[-1] / x.size()[-1]
        x = self.upsampling_method(x, skip_size)
        if self.use_skip:
            x = x + skip
        x = self.blend_conv.forward(x)
...
class _SimpleSegmentationModel(nn.Module):
    ...
    def forward(self, left_img, return_supcon_feature=False):
        # input_shape = left_img.shape[-2:]
        features = self.backbone(left_img)

        fine_feat = features['out']
        coarse_feat = features['low_level']

        if return_supcon_feature:
            bsz = fine_feat.shape[0]//2

            # split 'features'
            features['out'] = torch.split(fine_feat, [bsz, bsz], dim=0)[0] # [bsz, *]
            features['low_level'] = torch.split(coarse_feat, [bsz, bsz], dim=0)[0] # [bsz, *]

            fine_feat0 = features['out'] # features to use for 'pixel contrastive loss'
        else:
            fine_feat0 = fine_feat # [bsz, *]

        # print(features['low_level'].shape, features['out'].shape)
        # import sys; sys.exit(1)

        pred_segmap_beforeup = self.classifier(features) # predict segmentation
        
        # match fine_feat, fine_feat0 size(w,h) to pred_segmap_beforeup (for pixelcontrast loss)
        fine_feat0 = F.interpolate(fine_feat0, size=pred_segmap_beforeup.shape[-2:], mode='bilinear', align_corners=False)

        # final upsampling
        pred_segmap = F.interpolate(pred_segmap_beforeup, size=left_img.shape[2:], mode='bilinear', align_corners=False)
