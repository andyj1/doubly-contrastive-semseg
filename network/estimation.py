...
class DisparityEstimation(nn.Module):
    def __init__(self, max_disp, device, match_similarity=True):
    ...
    def forward(self, cost_volume):
        ...
        if cost_volume.size(1) == self.max_disp:
            disp_candidates = torch.arange(0, self.max_disp, device=self.device).type_as(prob_volume)
        else:
            max_disp = prob_volume.size(1)  # current max disparity
            disp_candidates = torch.arange(0, max_disp, device=self.device).type_as(prob_volume)
